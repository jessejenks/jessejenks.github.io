...
var train_button, reset_button, add_neuron_button, remove_neuron_button;
...
function preload() {
	...
	weight_colors = d3.interpolateRdYlGn;//interpolateSpectral;//
}
...
function draw() {
	...
	if (counter === data.length) {
		counter = 0;
		shuffleData();
	} else if (costs[costs.length-1] < 0.05) {
		currently_training = false;
		noLoop();
	}
}
