...
function setup() {
	...
	for (var i = 0; i<m; i++) {
		points[i] = {
			x: 3*Math.cos(TWO_PI*i/m),
			y: -3*Math.sin(TWO_PI*i/m)
		}
	}
}
...
function getGradient(c) {
	...
	var length = Math.sqrt(modulus_squared(z))

	var m = n + 1 - Math.log(Math.log(length))/log2;

	var f = Math.pow(2.0, -m);

	var dx = 0.5*(z.x*dz.x + z.y*dz.y);

	var dy = 0.5*(z.y*dz.x - z.x*dz.y)


	var den = length*length*Math.log(length)
	return {
		x: f*dx/den,
		y: f*dy/den
	} 
